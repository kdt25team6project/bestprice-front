...
function RecipeDetailPage() {
	...
	const handleToggleRecommend = async () => {
		...
		setIsLoading(true);
		try {
			const newRecommendState = await toggleRecommend(
				recipeId,
				userId,
				isRecommended
			);
		...
		const fetchRecipeData = async () => {
			try {
				...
				const recipeDetailResponse = await axios.get(
					`http://localhost:8001/api/recipe/${recipeId}`
				);
				const recipeExtraResponse = await axios.get(
					`http://localhost:8001/recipe?query=${recipeId}`
				);
		...
		<div className="recipe-container">
			...
			<div className="recipe-header">
				...
				<div className="recipeD-info">
					<span>
						<IoMdPerson /> {recipe.ckg_INBUN_NM}
					</span>
					<span>
						<MdOutlineAccessTime /> {recipe.ckg_TIME_NM}
					</span>
					<span>
						<SiLevelsdotfyi /> {recipe.ckg_DODF_NM}
					</span>
				</div>
			</div>
				...
				{recipe.steps.map((step, index) => (
					<li key={index}>
						...
						<div className="step_list_txt_pic">
							{step.stepsImg && (
								<img src={step.stepsImg} alt={`Step ${index + 1}`} />
							)}
						</div>
					</li>
				))}
