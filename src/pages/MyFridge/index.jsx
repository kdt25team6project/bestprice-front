...
const MyFridge = () => {
	...
	const totalChilledPages = Math.ceil(
		chilledItems.length / chilledItemsPerPage
	);
	...
	// 현재 페이지의 냉동/냉장 데이터
	const frozenCurrentItems = paginate(
		frozenItems,
		frozenPage,
		frozenItemsPerPage
	);
	const chilledCurrentItems = paginate(
		chilledItems,
		chilledPage,
		chilledItemsPerPage
	);
	...
	const chilledItemDistribution = distributeItemsForPage(
		chilledCurrentItems,
		18
	);
	...
	const currentItems = sortedFoodItems.slice(
		startIndex,
	...
	const paginationEnd = Math.min(
		totalPages,
		paginationStart + maxPageButtons - 1
	);
		...
		<div className="fridge-container">
			...
			<div className={`fridge-content ${!isLoggedIn ? "blurred" : ""}`}>
				<button
					...
					onClick={() => setIsLeftCanvasOpen(!isLeftCanvasOpen)}
				>
					추가하기
				</button>
				...
				{isTopDoorOpen && (
					<div className="pagination-frozen-left">
						<Pagination>
							...
							{/* 페이지 번호 */}
							{Array.from(
								{ length: Math.min(totalFrozenPages, 5) },
								(_, index) => {
				...
				{isBottomDoorOpen && (
					<div className="pagination-chilled-left">
						<Pagination>
							...
							{/* 페이지 번호 */}
							{Array.from(
								{ length: Math.min(totalChilledPages, 5) },
								(_, index) => {
				...
				<Modal show={showModal} onHide={handleCloseModal} centered>
					...
					<Modal.Body>
						{selectedFood && (
							<div>
								...
								<p>
									<strong>이름:</strong> {selectedFood.name}
								</p>
								<p>
									<strong>수량:</strong> {selectedFood.quantity}
								</p>
								<p>
									<strong>냉장/냉동:</strong>{" "}
									{selectedFood.is_frozen ? "냉동" : "냉장"}
								</p>
								<p>
									<strong>유통기한:</strong> {selectedFood.expiration_date}
								</p>
							</div>
						)}
					</Modal.Body>
					<Modal.Footer>
						<Button
							variant="danger"
							onClick={() => handleDelete(selectedFood.refrigerator)}
						>
							삭제
						</Button>
					</Modal.Footer>
				...
				>
					<div className="left-canvas-header">
						<h2 className="left-canvas-title">재료 추가</h2>
					...
					<div className="left-canvas-content">
						...
						{selectedEmoji && (
							<div className="selectedEmoji">{selectedEmoji}</div>
						)}
						{/* 카테고리 선택 버튼 */}
						...
						<div className="input-section">
							<p className="input-prompt">재료명</p>
							<input
								type="text"
							...
							<p className="input-prompt">냉장/냉동</p>
							<select
								className="input-field"
								value={frozen}
							...
							{recipe.slice(0, 5).map((recipeItem, index) => (
								...
								>
									<p className="recipe-name">
										식재료 : {ingredientName[index]}
									</p>
									<p className="recipe-title">
										{recipeItem?.rcp_TTL || "레시피 없음"}
									</p>
								</div>
							))}
						</div>
